...
void Cal(double MOne, double Epsilon)//Stuckelberg Model Constants, To avoid zero denominator, do not set MOne=MZ or Epsilon=0
{
  double CW2=1-SW2;
  double CW=sqrt(CW2);
  double SW=sqrt(SW2);
  ...
  double MOne2=MOne*MOne;
  double Epsilon2=Epsilon*Epsilon;
  double MZZp2=MZ2+MOne2*(1+Epsilon2);
  double Mplus2=0.5*(MZZp2+sqrt(MZZp2*MZZp2-4*MOne2*(MZ2+MW2*Epsilon2)));
  double MZp=sqrt(Mplus2);
  double Mminus2=0.5*(MZZp2-sqrt(MZZp2*MZZp2-4*MOne2*(MZ2+MW2*Epsilon2)));
  double tmp1=MOne2*Epsilon*(MW2-Mplus2)/MW2/TW/(MOne2-Mplus2);
  double tmp2=(MW2-Mplus2)/MW2/TW;
  ...
  double theStrengthOfWWcoupling=MZp>=2*sqrt(MW2)?(Mplus2/MW2)*sqrt((1+(1/TW2)*(1+Epsilon2))*TW)*R(2,0):0;
  ...
  char ss[255];
  sprintf(ss,"StuZp_%s_M%0.0f_Epsilon%0.2f_7TeV.py",name,MOne,Epsilon);
  string CardName(ss);
  CardName.erase(CardName.find(".")-1,2);
  ofstream PYCard(CardName.c_str());
  PYCard<<"import FWCore.ParameterSet.Config as cms"<<endl
	<<"source = cms.Source(\"EmptySource\")"<<endl
	<<"from Configuration.Generator.PythiaUESettings_cfi import *"<<endl
  ...
  for (int i=3;i>=0;i--)
    if (i!=2) {
      ...
      PYCard <<"'PARJ(190)="<< ((MZp>=2*Mt)?Vf(2)*sqrt(sqrt(1-4*Mt*Mt/Mplus2)*(1+2*Mt*Mt/Mplus2)):0) <<"   ! g_v for t to Zp',"<<endl;
      PYCard <<"'PARJ(191)="<< ((MZp>=2*Mt)?Af(2)*sqrt(sqrt(1-4*Mt*Mt/Mplus2)*(1-4*Mt*Mt/Mplus2)):0) <<"   ! g_a for t to Zp',"<<endl;
    }
